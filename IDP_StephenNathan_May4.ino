...
void loop() {
  ...
  if(fill_count >= BUF_SZ) {
    ...
    timestamped_val left_peak = {0};
    timestamped_val right_peak = {0};
    ...
    expX = (alpha*cos(radians(angle)))+((1-alpha)*prevExpX);
    prevExpX = expX;

    expY = (alpha*sin(radians(angle)))+((1-alpha)*prevExpY);
    prevExpY = expY;
    ...
    //Serial.print("Trig:");Serial.print(degrees(atan2(expY,expX)));Serial.print(" ");
    //Serial.println();

    display.clearDisplay();
    display.setCursor(20,20);
    displayAngleOled(expAverage);
  
    float calculated_angle_rad = (expAverage+90) * PI / 180.0;
    ...
    left[fill_count] = {left_voltage};
    right[fill_count] = {right_voltage};
    fill_count++;
